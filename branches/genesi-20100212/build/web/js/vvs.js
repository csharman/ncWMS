...
window.onload = function()
{
    ...
    omi_layer = new OpenLayers.Layer.WMS1_3(
            'OMI',
            'wms',
            {
                layers: 'OMI/Data_Fields:ColumnAmountO3',
                transparent: true,
                crs: 'CRS:84'
            },
            {
    ...
    map.addLayers([demis_wms, globmodel_layer, sciamachy_layer, omi_layer, coastline_wms]);
...
function updateVisibilities() {
    ...
    omi_layer.setVisibility($('omiVisibility').checked);
...
function updateOpacities() {
    ...
    omi_layer.setOpacity($('omiOpacity').value);
...
function updateColorScale() {
    ...
    omi_layer.mergeNewParams({ colorScaleRange: scaleMin * omiconv + ',' + scaleMax * omiconv });
...
function updateDates() {
    ...
    var omiDay = $('omiDay').value;
    var omiHour = $('omiHour').value;
    ...
    var omiDate = new Date();
    omiDate.setUTCFullYear(2006, 7, omiDay); // 7 = August



    if (omiHour==2)
    {
    	omiDate.setUTCHours(omiHour, 13, 21, 328); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==3)
    {
       	omiDate.setUTCHours(omiHour, 49, 48, 421); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==5)
    {
       	omiDate.setUTCHours(omiHour, 31, 8, 406); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==7)
    {
       	omiDate.setUTCHours(omiHour, 10, 1, 187); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==10)
    {
       	omiDate.setUTCHours(omiHour, 27, 47, 953); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==12)
    {
       	omiDate.setUTCHours(omiHour, 6, 40, 203); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==13)
    {
       	omiDate.setUTCHours(omiHour, 45, 31, 500); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==15)
    {
       	omiDate.setUTCHours(omiHour, 24, 24, 734); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==17)
    {
       	omiDate.setUTCHours(omiHour, 3, 17, 218); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==18)
    {
       	omiDate.setUTCHours(omiHour, 42, 9, 343); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==20)
    {
       	omiDate.setUTCHours(omiHour, 21, 2, 437); // Sets hours, minutes, seconds and ms
    }
    if (omiHour==21)
    {
       	omiDate.setUTCHours(omiHour, 59, 55, 562); // Sets hours, minutes, seconds and ms
    }

    var days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    $('dayOmiName').innerHTML = days[omiDate.getDay()];
    ...
    omi_layer.mergeNewParams({ time: dateToIso(omiDate) });
