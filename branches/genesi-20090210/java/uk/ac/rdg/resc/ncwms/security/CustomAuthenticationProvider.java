...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    private Authentication UserAuthenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (userId.equalsIgnoreCase("ASantokhee")) {
            ...
            //load cert for user logging into system
            String keyStorePath = "C:\\Tomcat 6.0\\webapps\\userscerts.p12";
            String keyStorePassword = "test123";

            //initialize keystore and truststore to get proxy cert from CSS
            SecureStore ss = new SecureStore(keyStorePath, keyStorePassword, webservice_url);
             
            //get ready to download proxy cert
            GetProxyCert gp = new GetProxyCert(ss.getHttpsConn());
            boolean resp = gp.ContactWebService(userId, "CyzpsnUZvOGreMp");

            if (resp) {

                //get proxy cert
                String proxyCert = gp.getProxyCertLocation();
     ...
     * @return
     */
    private Authentication UserAuthorize(Authentication authentication) throws AuthenticationException {
        String userId = authentication.getPrincipal().toString();
        String password = authentication.getCredentials().toString();
